...
def register_cashflow_tools(mcp):
    ...
    def cashflow(ts_code: str = "", ann_date: str = "", f_ann_date: str = "", start_date: str = "", 
        ...
        """
        获取上市公司现金流量表数据。支持多期趋势查询。

        参数:
            ts_code: 股票代码
            period: 报告期（精确匹配，指定后只返回该期单条）。趋势分析时请省略此参数，用 limit 替代
            report_type: 报告类型 (1合并报表 2单季合并)，趋势分析建议指定 report_type=1
        ...
        用法:
            - 多期趋势: cashflow(ts_code="300760.SZ", limit=12, report_type="1")
            - 查单期: cashflow(ts_code="300760.SZ", period="20251231")
        """
        ...
        if period and limit:
            return ("参数冲突：period（精确匹配单期）和 limit（多期趋势）不应同时使用。"
                    f"趋势分析请省略 period，例如：cashflow(ts_code=..., limit=12)；"
                    f"查单期请省略 limit，例如：cashflow(ts_code=..., period=\"20251231\")")
        params = {
            'ts_code': ts_code,
            'ann_date': ann_date,
