...
def register_weekly_tools(mcp):
    ...
    def weekly(ts_code: str = '', trade_date: str = '', start_date: str = '', end_date: str = '') -> str:
        ...
        df = pro.weekly(**api_params)
        
        if df.empty:
            return "未找到周线行情数据"

        results = [f"--- 周线行情数据 (Total: {len(df)}) ---"]
        ...
        for _, row in df_limited.iterrows():
             ...
             if pd.notna(row.get('open')): info.append(f"周开盘:{row['open']}")
             if pd.notna(row.get('high')): info.append(f"周最高:{row['high']}")
             if pd.notna(row.get('low')): info.append(f"周最低:{row['low']}")
             if pd.notna(row.get('close')): info.append(f"周收盘:{row['close']}")
             if pd.notna(row.get('pre_close')): info.append(f"上周收盘:{row['pre_close']}")
             if pd.notna(row.get('change')): info.append(f"周涨跌额:{row['change']}")
             if pd.notna(row.get('pct_chg')): info.append(f"周涨跌幅:{row['pct_chg']}%")
             if pd.notna(row.get('vol')): info.append(f"周成交量:{row['vol']}手")
             if pd.notna(row.get('amount')): info.append(f"周成交额:{row['amount']}千元")
